...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(input_size=embed_size + self.hidden_size,
                                   hidden_size=self.hidden_size,
                                   bias=True)
        # self.h_projection (Linear Layer with no bias)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # shape of X to (b, e, src_len). After getting the output from the CNN, still stored in the X variable,
        # remember to use torch.permute again to revert X back to its original shape.
        X = X.permute(1, 2, 0)  # from (src_len, b, e) to   (b, e, src_len)
        X = self.post_embed_cnn(X)
        X = X.permute(2, 0, 1)  # from  (b, e, src_len) to (src_len, b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 2. Use batched matrix multiplication between alpha_t and enc_hiddens to obtain the
        # attention output vector, a_t.
        a_t = torch.sum(alpha_t.reshape(b, src_len, 1) * enc_hiddens, dim=1)
        # 3. Concatenate dec_hidden with a_t to compute tensor U_t
